...
class pageView {
    ...
    private function search() {
        ...
        $this->dateRange = $_POST['date_range'];
        ...
        $str .= '<table class="wp-list-table widefat fixed posts view-count-table" id="view-count-table">';
            ...
            if(count($this->viewData) > 0) {
                foreach ($this->viewData as $key => $value) {
                    ...
                    $str .= '<td>'.date_i18n( get_option('date_format'), strtotime( $value->dDateAdded ) ).'</td>';
                    $str .= '</tr>';
        ...
        if(count($this->rawDetailData) > 0) {
            ...
            $innerTable .= '<thead>';
                $innerTable .= '<tr class="head" data-id="'.$_POST['id'].'" data-type="'.$_POST['type'].'">';
                    ...
                    $innerTable .= '<td align="center" width="25%" align="center"><a href="javascript:void(0);" class="sort-detail-data '.$classes['visit'].'" data-field="visit">Last Visit Date</a></th>';
            ...
            foreach($this->rawDetailData as $key=>$value) {
                $innerTable .= '<thead>';
                   $innerTable .= '<tr>';
                        ...
                        $innerTable .= '<td align="center">'.date_i18n( get_option('date_format'), strtotime( $value->dDateAdded ) ).'</td>';
                        $innerTable .= '<td> &nbsp; </td>';
                    $innerTable .= '</tr>';
                $innerTable .= '</thead>';
            } 
